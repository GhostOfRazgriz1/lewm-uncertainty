...
This module also serves as the importable RIG for M1.4 (src/surprise_head.py): the helpers below
(rollout, encode_all, _predict_one, tracking_errors, variance_sim, intrinsic_surprise, ...) plus the
loaded `model`/`prep`/`device` are module-level; the M1.3 experiment runs only under __main__.
"""
import sys
import numpy as np
import torch
...
T1 = T_STEPS + 1                                                  # number of latents / steps in a rollout
...
if __name__ == "__main__":
    ...
    fixed_b, fixed_e = run_lookset_arm(lambda K, ri, surp: fixed_interval_lookset(T1, K), rolls)
    rand_b, rand_e = run_lookset_arm(lambda K, ri, surp: random_lookset(T1, K, seed=ri), rolls)
    orac_b, orac_e = run_lookset_arm(lambda K, ri, surp: oracle_lookset(surp, K), rolls)
